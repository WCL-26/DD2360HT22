...
int main(int argc, char **argv) {
  ...
  cudaMallocManaged(&umA, numARows * numAColumns * sizeof(DataType)); 
  cudaMallocManaged(&umB, numBRows * numBColumns * sizeof(DataType)); 
  cudaMallocManaged(&umC, numCRows * numCColumns * sizeof(DataType)); 
  ...
  multiply(umA, umB, resultRef, numARows, numAColumns, numBRows, numBColumns);
  ...
  gemm<<<Dg,Db>>>(umA,umB,umC,numARows,numAColumns,numBRows,numBColumns);
  ...
  for (int i = 0; i < numCRows; ++i) {
      for (int j = 0; j < numCColumns; ++j) {
          if (fabs(umC[i*numCColumns + j] - resultRef[i*numCColumns + j]) > 1e-4){
